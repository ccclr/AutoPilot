...
use crate::messages::{
    Certificate, ConsensusMessage, ConsensusRequest, ConsensusVote, Header, Proposal, StateReport,
    Timeout, Vote, TC,
};
...
pub enum WorkerPrimaryMessage {
    ...
    OurBatch(Digest, WorkerId, config::BatchMetadata),
    /// The worker indicates it received a batch's digest from another authority.
    OthersBatch(Digest, WorkerId, config::BatchMetadata),
}
...
impl Primary {
    pub fn spawn(
        ...
        // Channel for receiving metrics state from metrics_collector
        let (tx_metrics_state, rx_metrics_state) = channel(CHANNEL_CAPACITY);
        ...
        let (tx_fast_sync_header_requests, rx_fast_sync_header_requests) =
        ...
        // // Extract node index from store path instead of committee ordering
        // info!("🔢 Primary {} extracting node index from store_path: {}", name, store_path);
        let node_index = extract_node_index_from_store_path(&store_path);
        //     .unwrap_or_else(|| {
        //         warn!("⚠️ Failed to extract node index from store_path '{}', falling back to committee ordering", store_path);
        //         committee.node_index(&name).unwrap_or(0) as usize
        //     });
        // info!("🔢 Primary {} found node index: {} (from store_path)", name, node_index);

        // Initialize the metrics logger with node index
        config::init_metrics_logger_with_node_index("logs", node_index);

        // Log committee information to metrics log for metrics_collector.py
        if let Some(mut logger_guard) = config::get_metrics_logger() {
            if let Some(logger) = logger_guard.as_mut() {
                for (pk, authority) in &committee.authorities {
                    let committee_details = serde_json::json!({
                        "pk": hex::encode(pk.0),
                        "stake": authority.stake,
                        "consensus_addr": authority.consensus.consensus_to_consensus.to_string()
                    });
                    logger.log_event("committee", committee_details);
                }
                info!("Logged committee information to metrics log");
            }
        }

        // Log public key and IP address to metrics log
        if let Some(mut logger_guard) = config::get_metrics_logger() {
            if let Some(logger) = logger_guard.as_mut() {
                logger.log_event("node_info", serde_json::json!({
                    "public_key": name,
                    "ip_address": committee.primary(&name).expect("Our public key or worker id is not in the committee").primary_to_primary.ip(),
                }));
            }
        }

        // Clone store_path for use by multiple components
        ...
        // Create channels for parameter updates
        let (tx_proposer_params, rx_proposer_params) = tokio::sync::mpsc::channel(1);
...
struct WorkerReceiverHandler {
    tx_our_digests: Sender<(Digest, WorkerId, config::BatchMetadata)>,
    tx_others_digests: Sender<(Digest, WorkerId, config::BatchMetadata)>,
}
