...
pub struct CommitQC {
    ...
    aggregate_report: Option<AggregateReport>,
}
...
pub fn transform_commitQC(commit_qc: CommitQC) -> ConsensusMessage {
    ConsensusMessage::Commit {
        ...
        aggregate_report: commit_qc.aggregate_report,
    }
}

fn prepare_digest(slot: Slot, view: View, aggregate_report: Option<&AggregateReport>) -> Digest {
    let mut hasher = Sha512::new();
    hasher.update(slot.to_le_bytes());
    hasher.update(view.to_le_bytes());
    if let Some(report) = aggregate_report {
        hasher.update(report.digest().0);
    }
    hasher.update((0 as u8).to_le_bytes());
    Digest(hasher.finalize().as_slice()[..32].try_into().unwrap())
}

fn confirm_digest(slot: Slot, view: View, prepare_id: &Digest) -> Digest {
    let mut hasher = Sha512::new();
    hasher.update(slot.to_le_bytes());
    hasher.update(view.to_le_bytes());
...
pub fn verify_commit(consensus_message: &ConsensusMessage, committee: &Committee) -> bool {
    match consensus_message {
        ConsensusMessage::Commit {
            ...
            let prepare_id = prepare_digest(*slot, *view, aggregate_report.as_ref());
    ...
    fn digest(&self) -> Digest {
        ...
        match self {
            ConsensusMessage::Prepare {
                ...
                // Include AggregateReport digest so that all honest nodes vote on
                // the same (slot, view, aggregate_report) triple.  A Byzantine
                // leader that equivocates on aggregate_report content would produce
                // a different Prepare digest, preventing conflicting QCs from
                // forming for the same (slot, view).
                return prepare_digest(*slot, *view, aggregate_report.as_ref());
            }
            ConsensusMessage::Confirm {
                slot,
                view,
            ...
            ConsensusMessage::Prepare {
                ...
                return match other {
                    ConsensusMessage::Prepare {
                        ...
                        aggregate_report: other_aggregate_report,
                    } => {
                        slot == other_slot
                            && view == other_view
                            && tc == other_tc
                            && proposals == other_proposals
                            && aggregate_report.as_ref().map(|r| r.digest())
                                == other_aggregate_report.as_ref().map(|r| r.digest())
                    }
                    _ => false,
                };
            }
...
pub struct Header {
    ...
    // Batch metadata for performance analysis
    pub batch_metadata: BTreeMap<Digest, config::BatchMetadata>,
}
...
impl Header {
    pub async fn new(
        ...
        batch_metadata: BTreeMap<Digest, config::BatchMetadata>,
...
impl StateReport {
    pub async fn new(
        ...
        let digest = Self::digest_for(epoch, &state);
    ...
    pub fn verify(&self, committee: &Committee) -> DagResult<()> {
        if committee.stake(&self.author) == 0 {
            debug!(
                "StateReport verify failed: unknown authority={}, epoch={}",
                self.author, self.epoch
            );
            return Err(DagError::UnknownAuthority(self.author));
        }

        let digest = Self::digest_for(self.epoch, &self.state);
        if let Err(e) = self.signature.verify(&digest, &self.author) {
            let dag_error = DagError::from(e);
            debug!(
                "StateReport verify failed: author={}, epoch={}, digest={}, error={}",
                self.author, self.epoch, digest, dag_error
            );
    ...
    pub fn digest(&self) -> Digest {
        Self::digest_for(self.epoch, &self.state)
    }

    fn digest_for(epoch: u64, state: &str) -> Digest {
        let mut hasher = Sha512::new();
...
impl fmt::Debug for StateReport {
    fn fmt(&self, f: &mut fmt::Formatter) -> Result<(), fmt::Error> {
        write!(
            f,
            "StateReport(epoch={}, author={})",
            self.epoch, self.author
        )
    }
}
...
impl AggregateReport {
    pub fn new(epoch: u64, mut reports: Vec<StateReport>) -> Self {
        reports.sort_by(|a, b| a.author.0.cmp(&b.author.0));
        Self { epoch, reports }
    }
    ...
    pub fn verify(&self, committee: &Committee, expected_epoch: u64) -> DagResult<()> {
        if self.epoch != expected_epoch {
            debug!(
                "AggregateReport verify failed: epoch mismatch report_epoch={}, expected_epoch={}, digest={}, reports={}",
                self.epoch,
                expected_epoch,
                self.digest(),
                self.reports.len()
            );
            return Err(DagError::InvalidQCTicket);
        }

        let mut seen_authors = HashSet::new();
        for report in &self.reports {
            ensure!(report.epoch == self.epoch, DagError::InvalidQCTicket);
            report.verify(committee)?;
            ensure!(
                seen_authors.insert(report.author),
                DagError::AuthorityReuse(report.author)
            );
        }
        Ok(())
    }
...
impl fmt::Debug for AggregateReport {
    fn fmt(&self, f: &mut fmt::Formatter) -> Result<(), fmt::Error> {
        write!(
            f,
            "AggregateReport(epoch={}, reports={})",
            self.epoch,
            self.reports.len()
        )
    }
